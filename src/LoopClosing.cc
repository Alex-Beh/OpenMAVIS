...
/******************************************************************************
 * Modified by:   Yifu Wang, Alvaro Parra * Contact:  1fwang927@gmail.com *
 ******************************************************************************/
      ...
      if (bFindedRegion && mbActiveLC)
      {
        if (mbMergeDetected)
        {
          if ((mpTracker->mSensor == System::IMU_MONOCULAR || mpTracker->mSensor == System::IMU_STEREO ||
               mpTracker->mSensor == System::IMU_RGBD || mpTracker->mSensor == System::IMU_MULTI) &&
...
bool LoopClosing::NewDetectCommonRegions()
{
  ...
  if (mnLoopNumCoincidences > 0)
  {
    ...
    Sophus::SE3d mTcl = (mpCurrentKF->GetPose(mLoopCameraPair.cameraID1) *
                         mpLoopLastCurrentKF->GetPoseInverse(mLoopCameraPair.cameraID1))
                            .cast<double>();
    g2o::Sim3 gScl(mTcl.unit_quaternion(), mTcl.translation(), 1.0);
    g2o::Sim3 gScw = gScl * mg2oLoopSlw;
    int numProjMatches = 0;
    vector<MapPoint*> vpMatchedMPs;
    bool bCommonRegion = DetectAndReffineSim3FromLastKF(mpCurrentKF, mpLoopMatchedKF, gScw, numProjMatches, mvpLoopMPs,
                                                        vpMatchedMPs, mLoopCameraPair);
    if (bCommonRegion)
    {
  ...
  {
    // Find from the last KF candidates
    Sophus::SE3d mTcl = (mpCurrentKF->GetPose(mMergeCameraPair.cameraID1) *
                         mpMergeLastCurrentKF->GetPoseInverse(mMergeCameraPair.cameraID1))
                            .cast<double>();
    ...
    bool bCommonRegion = DetectAndReffineSim3FromLastKF(mpCurrentKF, mpMergeMatchedKF, gScw, numProjMatches,
                                                        mvpMergeMPs, vpMatchedMPs, mMergeCameraPair);
    if (bCommonRegion)
    {
  ...
  if (!bLoopDetectedInKF && !vpLoopBowCand.empty())
  {
    mbLoopDetected = DetectCommonRegionsFromBoW(vpLoopBowCand, mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw,
                                                mnLoopNumCoincidences, mvpLoopMPs, mvpLoopMatchedMPs, mLoopCameraPair);
  }
  // Merge candidates
  if (!bMergeDetectedInKF && !vpMergeBowCand.empty())
  {
    mbMergeDetected =
        DetectCommonRegionsFromBoW(vpMergeBowCand, mpMergeMatchedKF, mpMergeLastCurrentKF, mg2oMergeSlw,
                                   mnMergeNumCoincidences, mvpMergeMPs, mvpMergeMatchedMPs, mMergeCameraPair);
  }
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(KeyFrame* pCurrentKF, KeyFrame* pMatchedKF, g2o::Sim3& gScw,
                                                 ...
                                                 std::vector<MapPoint*>& vpMatchedMPs, CameraMatch& mCameraPair)
{
  set<MapPoint*> spAlreadyMatchedMPs;
  nNumProjMatches =
      FindMatchesByProjection(pCurrentKF, pMatchedKF, gScw, spAlreadyMatchedMPs, vpMPs, vpMatchedMPs, mCameraPair);
  ...
  if (nNumProjMatches >= nProjMatches)
  {
    ...
    int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10, bFixedScale,
                                                mHessian7x7, true, mCameraPair.cameraID1, mCameraPair.cameraID2);
    ...
    if (numOptMatches > nProjOptMatches)
    {
      ...
      nNumProjMatches = FindMatchesByProjection(pCurrentKF, pMatchedKF, gScw_estimation, spAlreadyMatchedMPs, vpMPs,
                                                vpMatchedMPs, mCameraPair);
                                             ...
                                             CameraMatch& mCameraPair)
{
  ...
  for (KeyFrame* pKFi : vpBowCand)
  {
    ...
    if (numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
    {
      ...
      Sim3Solver solver = Sim3Solver(mpCurrentKF, pMostBoWMatchesKF, vpMatchedPoints, cameraID1, cameraID2, bFixedScale,
                                     vpKeyFrameMatchedMP);
      ...
      if (bConverge)
      {
        ...
        g2o::Sim3 gSmw(pMostBoWMatchesKF->GetRotation(cameraID2).cast<double>(),
                       pMostBoWMatchesKF->GetTranslation(cameraID2).cast<double>(), 1.0);
        ...
        int numProjMatches = matcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpKeyFrames, vpMatchedMP,
                                                        vpMatchedKF, 8, 1.5, cameraID1);
        ...
        if (numProjMatches >= nProjMatches)
        {
          ...
          int numOptMatches = Optimizer::OptimizeSim3(mpCurrentKF, pKFi, vpMatchedMP, gScm, 10, mbFixScale, mHessian7x7,
                                                      true, cameraID1, cameraID2);
          ...
          if (numOptMatches >= nSim3Inliers)
          {
            ...
            int numProjOptMatches =
                matcher.SearchByProjection(mpCurrentKF, mScw, vpMapPoints, vpMatchedMP, 5, 1.0, cameraID1);

            if (numProjOptMatches >= nProjOptMatches)
            {
              // do another refinement
              int numOptMatches2 = Optimizer::OptimizeSim3(mpCurrentKF, pKFi, vpMatchedMP, gScm, 10, mbFixScale,
                                                           mHessian7x7, true, cameraID1, cameraID2);
              g2o::Sim3 gSmw(pMostBoWMatchesKF->GetRotation(cameraID2).cast<double>(),
                             pMostBoWMatchesKF->GetTranslation(cameraID2).cast<double>(), 1.0);
              ...
              while (nNumKFs < 3 && j < vpCurrentCovKFs.size())
              {
                ...
                bool bValid = DetectCommonRegionsFromLastKF(pKFj, pMostBoWMatchesKF, gSjw, numProjMatches_j,
                                                            vpMapPoints, vpMatchedMPs_j, candidatePair);
                                         ...
                                         vector<MapPoint*>& vpMatchedMapPoints, CameraMatch& mCameraPair)
{
  ...
  int num_matches =
      matcher.SearchByProjection(pCurrentKF, mScw, vpMapPoints, vpMatchedMapPoints, 3, 1.5, mCameraPair.cameraID1);
...
void LoopClosing::MergeLocal()
{
  ...
  {
    unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate);  // We update the current map with the
                                                                   // Merge information
    unique_lock<mutex> mergeLock(pMergeMap->mMutexMapUpdate);      // We remove the Kfs and MPs in the merged
  ...
  else
  {
...

    {
      ...
      unique_lock<mutex> currentLock(pCurrentMap->mMutexMapUpdate);  // We update the current map with the
                                                                     // Merge information
      unique_lock<mutex> mergeLock(pMergeMap->mMutexMapUpdate);      // We remove the Kfs and MPs in the
...
void LoopClosing::MergeLocal2()
{
  ...
  vector<MapPoint*> vpCheckFuseMapPoint;  // MapPoint vector from current map to allow to fuse
