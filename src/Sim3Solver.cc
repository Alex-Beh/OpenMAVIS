...
Sim3Solver::Sim3Solver(KeyFrame* pKF1, KeyFrame* pKF2, const vector<MapPoint*>& vpMatched12, int& cameraID1,
                       int& cameraID2, const bool bFixScale, vector<KeyFrame*> vpKeyFrameMatchedMP)
  : mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale), pCamera1(pKF1->mpCamera), pCamera2(pKF2->mpCamera)
{
  ...
  for (int i1 = 0; i1 < mN1; i1++)
  {
    ...
    for (int c1 = 0; c1 < 4; c1++)
    {
      int idx1 = (c1 == 0) ?
                     std::get<0>(indices1) :
                     (c1 == 1) ? std::get<1>(indices1) : (c1 == 2) ? std::get<2>(indices1) : std::get<3>(indices1);
      if (idx1 < 0)
        continue;
      ...
      for (int c2 = 0; c2 < 4; c2++)
      {
        int idx2 = (c2 == 0) ?
                       std::get<0>(indices2) :
                       (c2 == 1) ? std::get<1>(indices2) : (c2 == 2) ? std::get<2>(indices2) : std::get<3>(indices2);
        if (idx2 < 0)
          continue;
  ...
  for (int i1 = 0; i1 < mN1; i1++)
  {
    if (vpMatched12[i1])
    {
      ...
      int indexKF1 = (cameraID1 == 0) ?
                         std::get<0>(pMP1->GetIndexInKeyFrame(pKF1)) :
      ...
      int indexKF2 = (cameraID2 == 0) ?
                         std::get<0>(pMP2->GetIndexInKeyFrame(pKFm)) :
