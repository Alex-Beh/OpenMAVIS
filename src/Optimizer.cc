      ...
      if (pMP)
      {
        ...
        else
        {
          ...
          else if (i >= pFrame->Nleft + pFrame->Nright && i < pFrame->Nleft + pFrame->Nright + pFrame->Nsideleft)
          {
            ...
            e->mTsll = g2o::SE3Quat(pFrame->GetRelativePoseTsll().unit_quaternion().cast<double>(),
                                    pFrame->GetRelativePoseTsll().translation().cast<double>());
          ...
          else
          {
            ...
            e->mTsrl = g2o::SE3Quat(pFrame->GetRelativePoseTsrl().unit_quaternion().cast<double>(),
                                    pFrame->GetRelativePoseTsrl().translation().cast<double>());
                                      ...
                                      int& num_MPs, int& num_edges)
{
  ...
  for (list<MapPoint*>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end(); lit != lend; lit++)
  {
    ...
    for (map<KeyFrame*, tuple<int, int, int, int>>::iterator mit = observations.begin(), mend = observations.end();
         mit != mend; mit++)
    {
      KeyFrame* pKFi = mit->first;
  ...
  vToErase.reserve(vpEdgesMono.size() + vpEdgesBody.size() + vpEdgesMono_SL.size() + vpEdgesMono_SR.size() +
                   vpEdgesStereo.size());
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2, vector<MapPoint*>& vpMatches1, g2o::Sim3& g2oS12,
                            ...
                            const bool bAllPoints, const int cameraID1, const int cameraID2)
{
  g2o::SparseOptimizer optimizer;
  g2o::BlockSolverX::LinearSolverType* linearSolver;
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    const int i2 = (cameraID2 == 0) ? get<0>(pMP2->GetIndexInKeyFrame(pKF2)) :
                                      (cameraID2 == 1) ?
                                      get<1>(pMP2->GetIndexInKeyFrame(pKF2)) :
                                      (cameraID2 == 2) ? get<2>(pMP2->GetIndexInKeyFrame(pKF2)) :
                                                         (cameraID2 == 3) ? get<3>(pMP2->GetIndexInKeyFrame(pKF2)) : -1;
                                ...
                                LoopClosing::KeyFrameAndPose& corrPoses)
{
  ...
  for (vector<pair<MapPoint*, int>>::iterator lit = pairs.begin(), lend = pairs.end(); lit != lend; lit++, i++)
  {
         ...
         mit != mend; mit++)
    {
      ...
      if (pKFi->mnBALocalForKF != pCurrKF->mnId && pKFi->mnBAFixedForKF != pCurrKF->mnId)  // If optimizable or already
                                                                                           // included...
      {
...

  {
    ...
    for (int i = 0; i < N; i++)
    {
      ...
      if (pMP)
      {
        ...
        if (bRight && (i >= Nleft + Nright + Nsideleft && i < Nleft + Nright + Nsideleft + Nsideright) &&
