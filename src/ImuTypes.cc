...
/******************************************************************************
 * Modified by:   Yifu Wang, Yonhon Ng * Contact:  1fwang927@gmail.com *
 ******************************************************************************/
                                            ...
                                            const float& dt)
{
  ...
  // rotation) Improved IMU Pre-integration
  ...
  J1 = dt * Eigen::Matrix3f::Identity() + (1.0f - cos(dt * theta)) / (theta2)*WaccW +
       (dt * theta - sin(dt * theta)) / (theta3)*WaccW2;
  J2 = 0.5f * dt * dt * Eigen::Matrix3f::Identity() + (dt * theta - sin(dt * theta)) / (theta3)*WaccW +
       (0.5f * dt * dt * theta2 + cos(dt * theta) - 1) / (theta4)*WaccW2;
