...
Sophus::SE3f Tracking::GrabImageMulti(const cv::Mat& imRectLeft, const cv::Mat& imRectRight,
                                      const cv::Mat& imRectSideLeft, const cv::Mat& imRectSideRight,
                                      const double& timestamp, string filename)
{
  ...
  if (mSensor == System::IMU_MULTI)
    mCurrentFrame = Frame(mImGray, imGrayRight, imGraySideLeft, imGraySideRight, timestamp, mpORBextractorLeft,
                          mpORBextractorRight, mpORBextractorSideLeft, mpORBextractorSideRight, mbleft, mbright,
                          mbsideleft, mbsideright, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, mpCamera2,
                          mpCamera3, mpCamera4, mTlr, mTlsl, mTlsr, &mLastFrame, *mpImuCalib);
  ...
  else
  {
    ...
    else if (mState == OK)
    {
          ...
          mSensor == System::IMU_MULTI)
      {
        ...
        // {
        //     cout << "IMU is not or recently initialized. Reseting active
        //     map..." << endl; mpSystem->ResetActiveMap();
        // }
  ...
  for (int i = 0; i < mCurrentFrame.N; i++)
  {
    ...
    if (mCurrentFrame.mvpMapPoints[i])
    {
      if (mCurrentFrame.mvbOutlier[i])
      {
        ...
        else if (i < mCurrentFrame.Nleft + mCurrentFrame.Nright + mCurrentFrame.Nsideleft &&
...
bool Tracking::NeedNewKeyFrame()
{
  ...
  const bool c1b = ((mCurrentFrame.mnId >= mnLastKeyFrameId + mMinFrames) && bLocalMappingIdle);  // mpLocalMapper->KeyframesInQueue()
                                                                                                  // < 2);
    ...
    for (int i = 0; i < mCurrentFrame.N; i++)
    {
      ...
      if (pMP)
      {
        if (!pMP->isBad())
        {
          ...
          for (map<KeyFrame*, tuple<int, int, int, int>>::const_iterator it = observations.begin(),
