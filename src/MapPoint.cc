  ...
  , mnOriginMapId(pMap->GetId())
{
  ...
  const int level = (pFrame->Nleft == -1) ?
                        ...
                        (idxF < pFrame->Nleft) ? pFrame->mvKeys[idxF].octave :
                                                 (idxF < pFrame->Nleft + pFrame->Nright) ?
                                                 pFrame->mvKeysRight[idxF].octave :
                                                 (idxF < pFrame->Nleft + pFrame->Nright + pFrame->Nsideleft) ?
                                                 pFrame->mvKeysSideLeft[idxF].octave :
...
void MapPoint::EraseObservation(KeyFrame* pKF)
{
  bool bBad = false;
  {
    ...
    if (mObservations.count(pKF))
    {
      ...
      int leftIndex = get<0>(indexes), rightIndex = get<1>(indexes), sideleftIndex = get<2>(indexes),
          siderightIndex = get<3>(indexes);
...
void MapPoint::PrintObservations()
{
  ...
  for (map<KeyFrame*, tuple<int, int, int, int>>::iterator mit = mObservations.begin(), mend = mObservations.end();
       mit != mend; mit++)
  {
...
void MapPoint::PreSave(set<KeyFrame*>& spKF, set<MapPoint*>& spMP)
{
  ...
  for (std::map<KeyFrame*, std::tuple<int, int, int, int>>::const_iterator it = mObservations.begin(),
...
void MapPoint::PostLoad(map<long unsigned int, KeyFrame*>& mpKFid, map<long unsigned int, MapPoint*>& mpMPid)
{
       ...
       it != end; ++it)
  {
    ...
    std::tuple<int, int, int, int> indexes =
        tuple<int, int, int, int>(it->second, it2->second, it3->second, it4->second);
    if (pKFi)
    {
