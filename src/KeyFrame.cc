...
KeyFrame::KeyFrame(Frame& F, Map* pMap, KeyFrameDatabase* pKFDB)
  ...
  , mpCamera3(F.mpCamera3)
  , mpCamera4(F.mpCamera4)
  ...
  , mTlsl(F.GetRelativePoseTlsl())
  , mTsll(F.GetRelativePoseTsll())
  , mTlsr(F.GetRelativePoseTlsr())
  , mTsrl(F.GetRelativePoseTsrl())
  , mvKeysRight(F.mvKeysRight)
  , mvKeysSideLeft(F.mvKeysSideLeft)
  , mvKeysSideRight(F.mvKeysSideRight)
  , NLeft(F.Nleft)
  , NRight(F.Nright)
  , NSideLeft(F.Nsideleft)
  , NSideRight(F.Nsideright)
  , mnNumberOfOpt(0)
  , mbHasVelocity(false)
{
  mnId = nNextId++;
...
Eigen::Matrix3f KeyFrame::GetRotation(const int cameraID)
{
  ...
  return (cameraID == 0) ?
             mRcw :
             (cameraID == 1) ?
             (mTrl.so3() * mTcw.so3()).matrix() :
             (cameraID == 2) ?
             (mTsll.so3() * mTcw.so3()).matrix() :
...
Eigen::Vector3f KeyFrame::GetTranslation(const int cameraID)
{
  ...
  return (cameraID == 0) ?
             mTcw.translation() :
             (cameraID == 1) ?
             (mTrl * mTcw).translation() :
             (cameraID == 2) ?
             (mTsll * mTcw).translation() :
...
vector<size_t> KeyFrame::GetFeaturesInArea(const float& x, const float& y, const float& r, const int cameraID) const
{
  ...
  for (int ix = nMinCellX; ix <= nMaxCellX; ix++)
  {
    ...
    {
      const vector<size_t> vCell = (cameraID == 0) ?
                                       mGrid[ix][iy] :
                                       (cameraID == 1) ?
                                       mGridRight[ix][iy] :
                                       (cameraID == 2) ?
      ...
      for (size_t j = 0, jend = vCell.size(); j < jend; j++)
      {
        ...
        if (fabs(distx) < r && fabs(disty) < r)
          vIndices.push_back((cameraID == 0) ?
                                 vCell[j] :
                                 (cameraID == 1) ?
                                 vCell[j] + NLeft :
                                 (cameraID == 2) ?
                                 vCell[j] + NLeft + NRight :
                                 (cameraID == 3) ?
                                 vCell[j] + NLeft + NRight + NSideLeft :
                                 throw std::invalid_argument("Invalid camera Idx: " + std::to_string(cameraID)));
      }
    }
  }
...
int KeyFrame::kpIdxToCamIdx(const int kpIdx) const
{
  return (NLeft == -1) ?
             0 :
             (kpIdx < NLeft) ? 0 : (kpIdx < NLeft + NRight) ? 1 : (kpIdx < NLeft + NRight + NSideLeft) ? 2 : 3;
}
