...
Sophus::SE3f System::TrackMulti(const cv::Mat& imLeft, const cv::Mat& imRight, const cv::Mat& imSideLeft,
                                const cv::Mat& imSideRight, const double& timestamp, const vector<IMU::Point>& vImuMeas,
                                string filename)
{
  ...
  Sophus::SE3f Twb =
      mpTracker->GrabImageMulti(imLeftToFeed, imRightToFeed, imSideLeftToFeed, imSideRightToFeed, timestamp, filename);
...
void System::SaveTrajectoryTUM(const string& filename)
{
       ...
       lit != lend; lit++, lRit++, lT++, lbL++)
  {
    ...
    f << setprecision(6) << *lT << " " << setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x()
      << " " << q.y() << " " << q.z() << " " << q.w() << endl;
  }
  f.close();
...
void System::SaveTrajectoryEuRoC(const string& filename)
{
       ...
       lit++, lRit++, lT++, lbL++)
  {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD || mSensor == IMU_MULTI)
    {
      ...
      f << setprecision(6) << (*lT) << " " << setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2) << " "
        << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
    else
    {
...
void System::SaveKeyFrameTrajectoryEuRoC(const string& filename)
{
  ...
  for (size_t i = 0; i < vpKFs.size(); i++)
  {
    ...
    if (mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO || mSensor == IMU_RGBD || mSensor == IMU_MULTI)
    {
      ...
      Eigen::Quaternionf q = Twb.unit_quaternion();
      Eigen::Vector3f twb = Twb.translation();
      f << setprecision(6) << pKF->mTimeStamp << " " << setprecision(9) << twb(0) << " " << twb(1) << " " << twb(2)
        << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
    }
    else
    {
