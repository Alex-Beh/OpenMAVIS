...
Frame::Frame(const Frame& frame)
  ...
  , mvKeysSideLeft(frame.mvKeysSideLeft)
  , mvKeysSideRight(frame.mvKeysSideRight)
  ...
  , mDescriptorsSideLeft(frame.mDescriptorsSideLeft.clone())
  , mDescriptorsSideRight(frame.mDescriptorsSideRight.clone())
  ...
  , mpCamera3(frame.mpCamera3)
  , mpCamera4(frame.mpCamera4)
  , Nleft(frame.Nleft)
  , Nright(frame.Nright)
  , Nsideleft(frame.Nsideleft)
  , Nsideright(frame.Nsideright)
  , monoLeft(frame.monoLeft)
  , monoRight(frame.monoRight)
  , monoSideLeft(frame.monoSideLeft)
  , monoSideRight(frame.monoSideRight)
  , mvLeftToRightMatch(frame.mvLeftToRightMatch)
  , mvRightToLeftMatch(frame.mvRightToLeftMatch)
  , mvLeftToSideLMatch(frame.mvLeftToSideLMatch)
  , mvSideLToLeftMatch(frame.mvSideLToLeftMatch)
  , mvRightToSideRMatch(frame.mvRightToSideRMatch)
  , mvSideRToRightMatch(frame.mvSideRToRightMatch)
    ...
    const cv::KeyPoint& kp = (Nleft == -1) ? mvKeysUn[i] :
                                             (i < Nleft) ? mvKeys[i] :
                                                           (i < rightBoundary) ? mvKeysRight[i - Nleft] :
                                                                                 (i < sideLeftBoundary) ?
                                                                                 mvKeysSideLeft[i - rightBoundary] :
...
vector<size_t> Frame::GetFeaturesInArea(const float& x, const float& y, const float& r, const int minLevel,
                                        const int maxLevel, const int selectedCamera) const
{
    ...
    for (int iy = nMinCellY; iy <= nMaxCellY; iy++)
    {
      ...
      for (size_t j = 0, jend = vCell.size(); j < jend; j++)
      {
        const cv::KeyPoint& kpUn = (Nleft == -1) ?
                                       ...
                                       mvKeysRight[vCell[j]] :
                                       (selectedCamera == 2) ?
                                       mvKeysSideLeft[vCell[j]] :
                                       (selectedCamera == 3) ? mvKeysSideRight[vCell[j]] : mvKeysUn[vCell[j]];
...

{
  ...
  std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
  thread threadLeft(&Frame::ExtractORB, this, 0, true, imLeft, static_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[0],
                    static_cast<KannalaBrandt8*>(mpCamera)->mvLappingArea[1]);
  thread threadRight(&Frame::ExtractORB, this, 1, true, imRight,
...
void Frame::ComputeMultiFishEyeMatches()
{
  ...
  if (mDescriptors.rows != 0 && mDescriptorsRight.rows != 0)
  {
    ...
    // TODO: for diffrent setup, sideward camera may have sufficient overlap for
    // stereo matching
    mvLeftToRightMatch = vector<int>(Nleft, -1);
    mvRightToLeftMatch = vector<int>(Nright, -1);
...
Frame::Frame(const cv::Mat& imLeft, const cv::Mat& imRight, const cv::Mat& imSideLeft, const cv::Mat& imSideRight,
             const double& timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight,
             ORBextractor* ORBextractorSideLeft, ORBextractor* ORBextractorSideRight, bool bleft, bool bright,
             bool bsideleft, bool bsideright, ORBVocabulary* voc, cv::Mat& K, cv::Mat& distCoef, const float& bf,
             const float& thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2, GeometricCamera* pCamera3,
             GeometricCamera* pCamera4, Sophus::SE3f& Tlr, Sophus::SE3f& Tlsl, Sophus::SE3f& Tlsr, Frame* pPrevF,
             const IMU::Calib& ImuCalib)
  : mpcpi(NULL)
  , mpORBvocabulary(voc)
  , mpORBextractorLeft(extractorLeft)
...

{
  ...
  std::chrono::steady_clock::time_point time_StartStereoMatches = std::chrono::steady_clock::now();
#endif
  // Here we assume sideward camera has no significant overlap with stereo
  // camera. we do not peform stereo matching between sideward and forward
