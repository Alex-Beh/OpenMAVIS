...
public:
  ...
  // Extract ORB on the image. 0 for left image and 1 for right image. 2 for
  // sideleft image and 3 for sideright image bool enableCamera used for switch
  // on / off arbitarty camera
  ...
  // For obtain relative pose between front-left (main camera) and side-left
  // camera
  ...
  // For obtain relative pose between front-left (main camera) and side-right
  // camera
  ...
  vector<size_t> GetFeaturesInArea(const float& x, const float& y, const float& r, const int minLevel = -1,
                                   const int maxLevel = -1, const int selectedCamera = 0) const;
...
public:
  ...
  // Feature extractor. The right is used only in the stereo case. Sideward
  // cameras are used for multi camera case
  ...
  // must be rectified. In the RGB-D case, RGB images can be distorted. In the
  // multi camera case, mvKeysSideLeft and mvKeysSideRight are for sideward
  // cameras.
...
public:
  ...
  // For sidewards matching, front-left camera only match side-left, front-right
  // camera only match side-right
  ...
  // Mutli cameras
  Frame(const cv::Mat& imLeft, const cv::Mat& imRight, const cv::Mat& imSideLeft, const cv::Mat& imSideRight,
        const double& timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight,
        ORBextractor* extractorSideLeft, ORBextractor* extractorSideRight, bool bEnableLeft, bool bEnableRight,
        bool bEnableSideLeft, bool bEnableSideRight, ORBVocabulary* voc, cv::Mat& K, cv::Mat& distCoef, const float& bf,
        const float& thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2, GeometricCamera* pCamera3,
        GeometricCamera* pCamera4, Sophus::SE3f& Tlr, Sophus::SE3f& Tlsl, Sophus::SE3f& Tlsr,
        Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib& ImuCalib = IMU::Calib());
